    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            if user_id in bgmi_cooldown:
                ...
                if time_since_last_attack < 60:
                    remaining_time = 60 - time_since_last_attack
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 120:
                ...
                response = f"@TOXICVENOMNO1 🚀\nAttack Finished. \n📡 Target: {target} \n🔌 Port: {port} \n⏱️ Time: {time} \n BY@TOXICVENOMNO1"
        else:
            response = "Usage :- /attack1 <target> <port> <time>\nBy @TOXICVENOMNO1"  # Updated command syntax
    else:
        response = "You Are Not User's To Authorized To Use This Command.\nBy STORM BOT DM TO GET ACCESS @TOXICVENOMNO1"
